...
<p>
The final high-level step began with the observation of SSH (Secure Shell) in Wireshark and Powershell of the Windows VM. I started the process by opening Wireshark, initiating a packet capture, and then filtering for exclusively SSH traffic. I then opened up Powershell and entered the command "ssh labuser@10.0.0.5," thereby joining the SSH command with the Linux username and private IP address. After issuing this command, I followed it by granting permission via the Linux password to access its VM through the Windows VM. I further observed in Wireshark that with each additional command, more and more encrypted packets came through between the server and the client. Lastly, I issued the final command of "exit" in Powershell to close the  Windows VM's access to the Linux VM.
</p>
<br />
